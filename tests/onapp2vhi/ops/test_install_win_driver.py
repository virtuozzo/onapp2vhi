...
class TestInstallWinDriverOffline(unittest.TestCase):
    ...
    @patch("onapp2vhi.ops.install_win_drivers.WindowsNetworkReconfig")
    @patch("onapp2vhi.inc.onapp_helpers.VmHandler")
    @patch("onapp2vhi.ops.install_win_drivers.ssh_run")
    @patch("onapp2vhi.ops.install_win_drivers.exit_status_code_handler")
    @patch("onapp2vhi.ops.install_win_drivers.SSH")
    @patch("onapp2vhi.ops.install_win_drivers.logs")
    @patch("onapp2vhi.ops.install_win_drivers.download_file")
    def test_vm_install_win_drivers_offline_with_bypass_host_key(self, mock_download, mock_logs,
                                                                 mock_ssh, mock_exit_status,
                                                                 mock_ssh_run, mock_vm_handler,
                                                                 mock_network_reconfig):
        ...
        result = vm_install_win_drivers(self.mock_cfg, mock_vm_handler, "aabbccdd", mock_properties)
