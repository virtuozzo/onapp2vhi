...
from onapp2vhi.inc.onapp_helpers import _spaces, onapp_version
...
class WindowsNetworkReconfig:
    ...
    def __init__(self, cfg: OnApp2VHIConfig, vm_identifier: str = ''):
        """
...
FOR /F "usebackq tokens=1-13,* delims=.-: " %%a IN (`"route print | C:\Windows\System32\\find.exe /I "{kwargs["mac_space"]}""`) DO netsh interface ipv6 add dns name=%%a {kwargs["dns"]}
    ...
    def _collect_vm_network_data(self):
        ...
        vm_networks = []
        vm_nics = get_virtual_server_interfaces(self.cfg, virtual_server_id=self.vm_identifier)

        for nic in vm_nics:
            _network = {}
            _nic = nic["network_interface"]
            _nic_id = _nic["id"]
            ip_addresses = get_virtual_server_ip_addresses(self.cfg,
                                                           virtual_server_id=self.vm_identifier,
            ...
            if _network['primary']:
                for num, _ip_addr in enumerate(_network['ip_addr_info']):
                    if _ip_addr['primary'] and not _ip_addr['ipv4']:
                        ...
                        _network['dns_ipv4'] = get_network_nameserver(self.cfg,
                                                                      network_id=network_id,
            ...
            for num, _ip_addr in enumerate(_network['ip_addr_info']):
                if _ip_addr["ipv4"]:
                    ...
                    _network['dns_ipv6'] = get_network_nameserver(self.cfg,
                                                                  network_id=network_id,
                                                                  ipv4=_network['ip_addr_info'][num]['ipv4'])
