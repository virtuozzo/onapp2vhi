...
Scenario: Hot migration without user's SSH key
  ...
  When I create a virtual machine (windows-vm-with-startup-cloudboot1)
  Then CP API (create) should return status code 201
  And I wait for 10 minutes
  And the virtual machine (windows-vm-with-startup-cloudboot1) is built successfully
  And the virtual machine (windows-vm-with-startup-cloudboot1) is booted successfully

  When I set the logging path (ultron_log/log)
  And I migrate the virtual machine (windows-vm-with-startup-cloudboot1)
  Then I wait for 10 seconds
  And I should see the virtual machine (windows-vm-with-startup-cloudboot1) is ACTIVE in VHI portal
...
Scenario: Hot migration with user's SSH key with storage policy and placement specified
  ...
  And I create a virtual machine (windows-vm-with-startup-cloudboot1)
  Then CP API (create) should return status code 201
  And I wait for 10 minutes
  And the virtual machine (windows-vm-with-startup-cloudboot1) is built successfully
  And the virtual machine (windows-vm-with-startup-cloudboot1) is booted successfully

  When I migrate the virtual machine (windows-vm-with-startup-cloudboot1) with following details
  | storage policy        | placement             |
  | behave-storage-policy | behave-soft-placement |
  Then I wait for 10 seconds
  And I should see the virtual machine (windows-vm-with-startup-cloudboot1) is ACTIVE in VHI portal
  And the virtual machine (windows-vm-with-startup-cloudboot1) should have correct CPU, RAM and storage
  And the virtual machine (windows-vm-with-startup-cloudboot1) is using the correct storage policy (behave-storage-policy) in its volume
  And the virtual machine (windows-vm-with-startup-cloudboot1) is placed in the corrent placement (behave-soft-placement)
...
Scenario: Hot migration with user's SSH key with second network interface (IPv4 and IPv6)
  ...
  When I add a network interface (behave-network-interface-ipv4-ipv6) with network join (behave-network-join-997) at compute zone (CloudBoot Compute Zone) to the virtual machine (windows-vm-with-startup-cloudboot1)
  Then CP API (create) should return status code 201

  When I add an IP address (behave-ip-net-ipv4) from network (behave-network-ipv4-ipv6) to the network interface (behave-network-interface-ipv4-ipv6) on virtual machine (windows-vm-with-startup-cloudboot1)
  Then CP API (create) should return status code 201
  
  When I add an IP address (behave-ip-net-ipv6) from network (behave-network-ipv4-ipv6) to the network interface (behave-network-interface-ipv4-ipv6) on virtual machine (windows-vm-with-startup-cloudboot1)
  Then CP API (create) should return status code 201

  When I reboot the virtual machine (windows-vm-with-startup-cloudboot1) in Onapp cloud
  Then CP API (reboot) should return status code 201
  And I wait for 60 seconds

  When I migrate the virtual machine (windows-vm-with-startup-cloudboot1)
  Then I wait for 10 seconds
  And I should see the virtual machine (windows-vm-with-startup-cloudboot1) is ACTIVE in VHI portal
  And the virtual machine (windows-vm-with-startup-cloudboot1) should have correct CPU, RAM and storage
