...
Scenario: Cold migration without user's SSH key
  ...
  When I create a virtual machine (windows-vm-without-startup-static)
  Then CP API (create) should return status code 201
  And I wait for 10 minutes
  And the virtual machine (windows-vm-without-startup-static) is built successfully

  When I migrate the virtual machine (windows-vm-without-startup-static)
...
Scenario: Cold migration with user's SSH key
  ...
  And I migrate the virtual machine (windows-vm-without-startup-static)
  Then I wait for 10 seconds
  And I should see the virtual machine is SHUTOFF in VHI portal
  And its CPU, RAM and storage are correct
...
Scenario: Cold migration with user's SSH key with storage policy specified
  ...
  And I create a virtual machine (windows-vm-without-startup-static)
  Then CP API (create) should return status code 201
  And I wait for 10 minutes
  And the virtual machine (windows-vm-without-startup-static) is built successfully

  When I migrate the virtual machine (windows-vm-without-startup-static) with following details
  | storage policy        |
...
Scenario: Cold migration with user's SSH key with second network interface (IPv4)
  ...
  When I add a network interface (behave-network-interface-ipv4) with network join (behave-network-join-993) at compute zone (Static Compute Zone) to the virtual machine (windows-vm-without-startup-static)
  Then CP API (create) should return status code 201

  When I add an IP address (behave-ip-net-ipv4) from network (behave-network-ipv4) to the network interface (behave-network-interface-ipv4) on virtual machine (windows-vm-without-startup-static)
  Then CP API (create) should return status code 201

  When I reboot the virtual machine (windows-vm-without-startup-static) in Onapp cloud
  Then CP API (reboot) should return status code 201
  And I wait for 90 seconds

  When I shutdown the virtual machine (windows-vm-without-startup-static) in Onapp cloud
...
Scenario: Cold migration with user's SSH key with second network interface (IPv4 and IPv6)
  ...
  When I add a network interface (behave-network-interface-ipv4-ipv6) with network join (behave-network-join-997) at compute zone (Static Compute Zone) to the virtual machine (windows-vm-without-startup-static)
  Then CP API (create) should return status code 201

  When I add an IP address (behave-ip-net-ipv4) from network (behave-network-ipv4-ipv6) to the network interface (behave-network-interface-ipv4-ipv6) on virtual machine (windows-vm-without-startup-static)
  Then CP API (create) should return status code 201
  
  When I add an IP address (behave-ip-net-ipv6) from network (behave-network-ipv4-ipv6) to the network interface (behave-network-interface-ipv4-ipv6) on virtual machine (windows-vm-without-startup-static)
  Then CP API (create) should return status code 201
